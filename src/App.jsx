  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <h3 style={{ margin: "0 0 14px", fontSize: 14, color: "#94a3b8" }}>Heures par activité</h3>
        ...
        {byActivity.map(a => (
          <div key={a.activity} style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <span style={{ color: "#64748b", fontSize: 11 }}>{a.count} séance{a.count>1?"s":""}</span>
            ...
            <div key={e.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "10px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0, marginLeft: 8 }}>
                  ...
                  ) : <span style={{ color: "#e2e8f0", fontWeight: 700, fontSize: 14 }}>{e.hours}h</span>}
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        {coachStats.length === 0 && <p style={{ color: "#334155", fontSize: 13 }}>Aucune donnée</p>}
          ...
          {coachStats.map(c => (
            <div key={c.name}>
              ...
              <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 99, height: 7 }}>
                <div style={{ background: "linear-gradient(90deg,#f59e0b,#ef4444)", height: "100%", borderRadius: 99, width: `${coachStats.length && Math.max(...coachStats.map(x=>x.hours),1) > 0 ? (c.hours/Math.max(...coachStats.map(x=>x.hours),1))*100 : 0}%`, transition: "width .4s" }} />
              </div>
            </div>
          ))}
        </div>
          ...
          {Array(days).fill(null).map((_, i) => {
            ...
            return (
              <div key={day}
                ...
                style={{ minHeight: 48, borderRadius: 6, background: isSel ? "rgba(59,130,246,0.2)" : hasEvents ? "rgba(255,255,255,0.03)" : isToday ? "rgba(59,130,246,0.08)" : "rgba(255,255,255,0.01)", border: isSel ? "2px solid rgba(59,130,246,0.6)" : isToday ? "1px solid rgba(59,130,246,0.3)" : "1px solid rgba(255,255,255,0.05)", padding: "3px", cursor: canEditDays ? "pointer" : "default", position: "relative", transition: "all .1s" }}>
                ...
                {/* Bouton édition séparé pour ne pas bloquer la sélection */}
                {hasEvents && canEditDays && (
                  <div
                    onClick={ev => { ev.stopPropagation(); onEditDay(ds, dayEvents); }}
                    style={{ position: "absolute", top: 1, left: 1, fontSize: 9, color: "#60a5fa", cursor: "pointer", background: "rgba(59,130,246,0.2)", borderRadius: 3, width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}
                    title="Modifier les activités de ce jour">
                    ✏
                  </div>
                )}
              </div>
...
function DayEditModal({ date, dayEvents, onClose, onDelete, onUpdate, onAdd, coaches }) {
  ...
  const [newActivity, setNewActivity] = useState("Entraînement");
  const [newCategories, setNewCategories] = useState([]);
  const [newCoachId, setNewCoachId] = useState(coaches.length > 0 ? coaches[0].id : null);
  const [newHours, setNewHours] = useState("2");
  const [newNote, setNewNote] = useState("");

  const isCourseNew = newActivity === "Course";
  const canSaveNew = newCategories.length > 0 && (!isCourseNew || newNote.trim().length > 0) && newHours && newCoachId;

  const toggleNewCat = (c) => setNewCategories(p => p.includes(c) ? p.filter(x => x !== c) : [...p, c]);

  const handleAdd = () => {
    const coach = coaches.find(c => c.id === newCoachId);
    if (!coach) return;
    onAdd({ activity: newActivity, categories: newCategories, hours: parseInt(newHours), note: newNote, coachId: newCoachId, coachName: coach.name });
    setShowAddForm(false);
    setNewActivity("Entraînement");
    setNewCategories([]);
    setNewHours("2");
    setNewNote("");
  };

  const fStyle = { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "8px 12px", color: "#e2e8f0", fontSize: 13, outline: "none", fontFamily: "inherit", width: "100%", boxSizing: "border-box" };

  const startEdit = (e) => { setEditingId(e.id); setEditCoachId(e.coachId); setEditHours(String(e.hours)); setShowAddForm(false); };
  const handleUpdate = (e) => { const coach = coaches.find(c => c.id === editCoachId); onUpdate(e.id, { coachId: editCoachId, coachName: coach ? coach.name : e.coachName, hours: parseInt(editHours) }); setEditingId(null); };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: "#161f2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "20px 20px 0 0", padding: 24, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
              ...
              {editingId === e.id ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <Badge activity={e.activity} />
                    {e.categories && e.categories.map(c => <span key={c} style={{ background: "rgba(59,130,246,0.15)", color: "#60a5fa", padding: "2px 7px", borderRadius: 99, fontSize: 10, fontWeight: 600 }}>{c}</span>)}
                  </div>
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {coaches.map(c => {
                        ...
                        return <div key={c.id} onClick={() => setEditCoachId(c.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, cursor: "pointer", border: `1px solid ${sel ? "#3b82f6" : "rgba(255,255,255,0.06)"}`, background: sel ? "rgba(59,130,246,0.12)" : "rgba(255,255,255,0.02)" }}>
                          <div style={{ width: 16, height: 16, borderRadius: "50%", background: sel ? "#3b82f6" : "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{sel && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff" }} />}</div>
                  ...
                  <div>
                    ...
                    <select value={editHours} onChange={ev => setEditHours(ev.target.value)} style={fStyle}>
                      {[1,2,3,4,5,6,7,8].map(h => <option key={h} value={h}>{h}h</option>)}
                    </select>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
        ...
        {showAddForm ? (
          <div style={{ background: "rgba(59,130,246,0.05)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 14, padding: 16, marginBottom: 14 }}>
            <h4 style={{ margin: "0 0 14px", fontSize: 14, color: "#60a5fa" }}>➕ Nouvelle entrée</h4>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {/* Activité */}
              <div>
                <label style={{ display: "block", color: "#64748b", fontSize: 10, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Activité</label>
                <div style={{ display: "flex", gap: 8 }}>
                  {["Entraînement","Course"].map(a => {
                    const col = ACT_COLORS[a] || { bar: "#3b82f6", bg: "rgba(59,130,246,0.15)", text: "#60a5fa" };
                    const active = newActivity === a;
                    return <div key={a} onClick={() => { setNewActivity(a); setNewCategories([]); setNewNote(""); }} style={{ flex: 1, textAlign: "center", padding: "9px 6px", borderRadius: 10, cursor: "pointer", border: `2px solid ${active ? col.bar : "rgba(255,255,255,0.08)"}`, background: active ? col.bg : "rgba(255,255,255,0.02)", color: active ? col.text : "#475569", fontWeight: active ? 700 : 400, fontSize: 13, userSelect: "none" }}>{a}</div>;
                  })}
                </div>
              </div>
              {/* Catégories */}
              <div>
                <label style={{ display: "block", color: "#64748b", fontSize: 10, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Catégories <span style={{ color: "#ef4444" }}>*</span></label>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(60px, 1fr))", gap: 5 }}>
                  {CATEGORIES.map(cat => {
                    const sel = newCategories.includes(cat);
                    return <div key={cat} onClick={() => toggleNewCat(cat)} style={{ textAlign: "center", padding: "7px 4px", borderRadius: 8, cursor: "pointer", border: `2px solid ${sel ? "#3b82f6" : "rgba(255,255,255,0.08)"}`, background: sel ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.02)", color: sel ? "#60a5fa" : "#475569", fontSize: 11, fontWeight: sel ? 700 : 400, userSelect: "none" }}>{cat}</div>;
                  })}
                </div>
              </div>
              {/* Entraîneur */}
              <div>
                <label style={{ display: "block", color: "#64748b", fontSize: 10, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Entraîneur <span style={{ color: "#ef4444" }}>*</span></label>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {coaches.map(c => {
                    const sel = newCoachId === c.id;
                    return <div key={c.id} onClick={() => setNewCoachId(c.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, cursor: "pointer", border: `1px solid ${sel ? "#3b82f6" : "rgba(255,255,255,0.06)"}`, background: sel ? "rgba(59,130,246,0.12)" : "rgba(255,255,255,0.02)" }}>
                      <div style={{ width: 16, height: 16, borderRadius: "50%", background: sel ? "#3b82f6" : "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{sel && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff" }} />}</div>
                      <span style={{ color: sel ? "#e2e8f0" : "#64748b", fontSize: 12, fontWeight: sel ? 600 : 400 }}>🎿 {c.name}</span>
                    </div>;
                  })}
              ...
              {/* Heures */}
              <div>
                <label style={{ display: "block", color: "#64748b", fontSize: 10, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Heures <span style={{ color: "#ef4444" }}>*</span></label>
                <select value={newHours} onChange={e => setNewHours(e.target.value)} style={fStyle}>
                  {[1,2,3,4,5,6,7,8].map(h => <option key={h} value={h}>{h}h</option>)}
                </select>
              </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <Btn onClick={handleAdd} disabled={!canSaveNew} variant="success" style={{ flex: 1 }}>✓ Ajouter</Btn>
                <Btn variant="ghost" onClick={() => setShowAddForm(false)}>Annuler</Btn>
              </div>
            </div>
          </div>
        ) : (
          <Btn onClick={() => { setShowAddForm(true); setEditingId(null); }} variant="primary" style={{ width: "100%", marginBottom: 10 }}>
    ...
    <div>
      ...
      {editDayDate && editDayEvents.length > 0 && (
        <DayEditModal
          ...
          onAdd={async ({ activity, categories, hours, note, coachId, coachName }) => {
            await dbOps.addCalEvent({ coachId, coachName, activity, date: editDayDate, categories, hours, note });
          }}
        />
      )}
